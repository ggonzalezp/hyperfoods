...
for i in range(5):
            ...
            for do_layers in [1,2]:
                for k_hops in [2,4,6]:
                    for bn in [True, False]:
                        ...
                        #MODEL DEFINITION
                        model = ChebModel(in_channels, n_classes, args.num_layers,
                                         args.hidden_gcn, args.hidden_fc, edge_index, mode=args.feature_agg_mode,batchnorm=bn, k_hops = k_hops, do_layers = do_layers).to(device)
    ...
    ###Eval on test set
    model = ChebModel(in_channels, n_classes, args.num_layers, args.hidden_gcn, args.hidden_fc, edge_index, mode=args.feature_agg_mode, batchnorm=best_bn, k_hops = best_k_hops, do_layers = best_do).to(device)

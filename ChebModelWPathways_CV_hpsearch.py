...
from models.networks_gcn import ChebModelWPathways
...
parser.add_argument('--checkpoint_key', default=None, type=str, help='the name of the checkpoint file (supporting partial name)')
...
for i in range(5):
            ...
            for do_layers in [1,2]:
                for k_hops in [2,4,6]:
                    for bn in [True, False]:
                        args.checkpoints_dir = out_dir_ii = out_dir_i + '/lr_{}_l2_{}_do_{}_k_{}_bn_{}/checkpoints'.format(lr,l2, do_layers, k_hops, bn)
                        utils.makedirs(args.checkpoints_dir)
                        out_dir_ii = out_dir_i + '/lr_{}_l2_{}_do_{}_k_{}_bn_{}'.format(lr,l2, do_layers, k_hops, bn)
                        writer = Writer(args, outdir=out_dir_ii)

                    # Defining model
                        model = ChebModelWPathways(in_channels, n_classes, args.num_layers,
                                                  args.hidden_gcn, args.hidden_fc, pathway_edge_index.to(device), n_cmt, edge_index,
                                                  mode=args.feature_agg_mode, batchnorm=bn, k_hops = k_hops, do_layers = do_layers).to(device)
    ...
    ###Eval on test set
    model = ChebModelWPathways(in_channels, n_classes, args.num_layers, args.hidden_gcn, args.hidden_fc, pathway_edge_index.to(device), n_cmt, edge_index, mode=args.feature_agg_mode, batchnorm=best_bn, k_hops = best_k_hops, do_layers = best_do).to(device)

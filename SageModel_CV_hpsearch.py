...
from models.networks_gcn import SageModel
...
for i in range(5):
        ...
        for l2 in [0.00001, 0.0001, 0.0005]:
            for do_layers in [1,2]:
                for bn in [True, False]:
                    ...
                    #MODEL DEFINITION
                    model = SageModel(in_channels, n_classes, args.num_layers,
                                     args.hidden_gcn, args.hidden_fc, edge_index, mode=args.feature_agg_mode,batchnorm=bn, do_layers = do_layers).to(device)
    ...
    ###Eval on test set
    model = SageModel(in_channels, n_classes, args.num_layers, args.hidden_gcn, args.hidden_fc, edge_index, mode=args.feature_agg_mode, batchnorm=best_bn, do_layers = best_do).to(device)

...
def preparing_data(drug_indices_of_interest, dataset_dir, outdir, best_model_path, norm, do_layers, batchnorm, num_layers, hidden_gcn, hidden_fc, splits_dir, in_channels, n_classes, feature_names_path, model_type, k_hops):
    ...
    # Loads datasets
    drug_fp = osp.join(dataset_dir, 'processed', 'new', 'raw_ac.py')
    pathway_fp = osp.join(dataset_dir, 'processed', 'pathway_kegg.py')
    ac_split_fp = sorted(glob(osp.join(splits_dir,'split_cancer_*_indices_approved_mysql_on_onecc_noiso_string_dense.hkl')))

    drug_ids = osp.join(dataset_dir, 'data_hyperfoods_drugcentral/drugs_ids_approved_mysql_on_onecc_noiso_string_dense.hkl')
    ...
    loader = DataLoader(samples_of_interest, batch_size=8)
...
def preparing_data_foods(drug_indices_of_interest, dataset_dir, outdir, best_model_path, norm, do_layers, batchnorm, num_layers, hidden_gcn, hidden_fc, splits_dir, in_channels, n_classes, feature_names_path, model_type, k_hops):
    device = torch.device('cpu')
    ...
    # Loads datasets
    drug_fp = osp.join(dataset_dir, 'processed', 'new', 'raw_foods.py')
    ...
    loader = DataLoader(samples_of_interest, batch_size=20)
    feature_names = hkl.load(feature_names_path)
    #Capitalizing feature names
    feature_names = [e.upper() for e in feature_names]
...
def preparing_data_all_positive_in_test(dataset_dir, outdir, best_model_path, norm, do_layers, batchnorm, num_layers, hidden_gcn, hidden_fc, splits_dir, in_channels, n_classes, feature_names_path, model_type, k_hops):
    device = torch.device('cpu')
...
##Computes embeddings for all samples in drug_indices_of_interest and returns as data frame
def get_embeddings_to_interpret_all_positive_in_test(dataset_dir, outdir, best_model_path, norm, do_layers, batchnorm, num_layers, hidden_gcn, hidden_fc, splits_dir, in_channels, n_classes, feature_names_path, model_type, k_hops):

    model, samples_of_interest, loader, feature_names, device = preparing_data_all_positive_in_test(dataset_dir, outdir, best_model_path, norm, do_layers, batchnorm, num_layers, hidden_gcn, hidden_fc, splits_dir, in_channels, n_classes, feature_names_path, model_type, k_hops)
    ...
    with torch.no_grad():
        ...
        embeddings = pd.DataFrame(np.vstack(list_embeddings))
...
def get_attributions_to_interpret_all_positive_in_test(dataset_dir, outdir, best_model_path, norm, do_layers, batchnorm, num_layers, hidden_gcn, hidden_fc, splits_dir, in_channels, n_classes, feature_names_path, model_type, k_hops, n_steps = 100):
    model, samples_of_interest, loader, feature_names, device = preparing_data_all_positive_in_test(dataset_dir, outdir, best_model_path, norm, do_layers, batchnorm, num_layers, hidden_gcn, hidden_fc, splits_dir, in_channels, n_classes, feature_names_path, model_type, k_hops)
...
def get_gsea_reports(data_frame, outdir, name, gsea_path = './'):
    ...
    for i in range(data_frame.shape[0]):
        ...
        command = 'bash {}GSEA_Linux_4.0.3/gsea-cli.sh GSEAPreranked -rnk {}/list.rnk -gmx ./GSEA_Linux_4.0.3/c2.cp.kegg.v7.0.symbols.gmt -nperm 1000  -out {} -rpt_label {}_KEGG'.format(gsea_path, outdir_i, outdir_i, name)
        subprocess.call(command, shell = True)

        # command = 'bash {}GSEA_Linux_4.0.3/gsea-cli.sh GSEAPreranked -rnk {}/list.rnk -gmx ./GSEA_Linux_4.0.3/c2.cp.reactome.v7.0.symbols.gmt -nperm 1000  -out {} -rpt_label {}_REACTOME'.format(gsea_path, outdir_i, outdir_i, name)
        # subprocess.call(command, shell = True)
...
def get_gsea_reports_food(data_frame, outdir, sample_ids, name, gsea_path = './'):
    ...
    for i in range(data_frame.shape[0]):
        ...
        outdir_i = outdir + '/{}'.format(sample_ids[i])
        if not os.path.isdir(outdir_i):
            os.makedirs(outdir_i)
        gene_ranked_dataframe.to_csv(outdir_i+'/list.rnk', sep = '\t', index = False)


        command = 'bash {}GSEA_Linux_4.0.3/gsea-cli.sh GSEAPreranked -rnk {}/list.rnk -gmx {}GSEA_Linux_4.0.3/c2.cp.kegg.v7.0.symbols.gmt -nperm 1000  -out {} -rpt_label {}_KEGG'.format(gsea_path, outdir_i, gsea_path, outdir_i, name)
        subprocess.call(command, shell = True)

        command = 'bash {}GSEA_Linux_4.0.3/gsea-cli.sh GSEAPreranked -rnk {}/list.rnk -gmx {}GSEA_Linux_4.0.3/c2.cp.reactome.v7.0.symbols.gmt -nperm 1000  -out {} -rpt_label {}_REACTOME'.format(gsea_path, outdir_i, gsea_path, outdir_i, name)
        subprocess.call(command, shell = True)
...
def get_ratio_OR_pathways(indir, name):
    dict_results = {}

    #Pathway info
    pathway_info = pd.read_csv('pathways_info.csv', sep = ',')  #Assignation matrix indicating which pathways are anticancer
    ...
    #Paths
    paths = sorted(glob(indir + '/{}.GseaPreranked.*/gsea_report_for_na_*.xls'.format(name)))
    neg_path = paths[0]
    pos_path = paths[1]
...
##Returns indices of correctly classified samples in the test set (index referring to the index of the samples after the attributions have been computed (index in the folder))
def get_index_correct_in_test(best_model_path):

        #Getting samples of interest in test set (classified as positive)
        test_pred = pd.read_csv(osp.join(best_model_path, '../best_model_results/test_pred.csv')).values.squeeze()
        drug_indices_of_interest = np.where(test_pred)[0].tolist()

        test_real = pd.read_csv(osp.join(best_model_path, '../best_model_results/test_real.csv')).values.squeeze()

        correctly_classified = np.nonzero(np.logical_and(test_pred == test_real, test_real == 1))[0]

        indices_in_testi = []
        for i in correctly_classified:
            indices_in_testi.append(drug_indices_of_interest.index(i))

        return indices_in_testi

...
from models.networks_gcn import SageModelWPathways
...
for i in range(5):
    ...
    for lr in [0.0005, 0.005]:
        for l2 in [0.00001, 0.0001, 0.0005]:
            for do_layers in [1,2]:
                ...
                # Defining model
                    model = SageModelWPathways(in_channels, n_classes, args.num_layers,
                                              args.hidden_gcn, args.hidden_fc, pathway_edge_index.to(device), n_cmt, edge_index,
                                              mode=args.feature_agg_mode, batchnorm=bn, do_layers=do_layers).to(device)
    ...
    ###Eval on test set
    model = SageModelWPathways(in_channels, n_classes, args.num_layers, args.hidden_gcn, args.hidden_fc, pathway_edge_index.to(device), n_cmt, edge_index, mode=args.feature_agg_mode, batchnorm=best_bn, do_layers=best_do).to(device)
